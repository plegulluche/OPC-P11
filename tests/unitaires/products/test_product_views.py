...
def test_product_page():
    ...
    Nutrimage.objects.create(name="a", link="fsefseg")
    client = Client()
...
def test_search_result_page():
    ...
    assert (
        len(response.context[0]["products"])
        == Product.objects.filter(Q(name__icontains=q)).order_by("name")[:10].count()
    )
...
def test_substitution_results_view():
    ...
    client = Client()
    response = client.get(
        reverse("substitute_results", kwargs={"productid": product1.id})
    )
    assert response.status_code == 200
